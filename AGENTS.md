...
6. Before source edits run `git status --short`; after edits run `git diff --check` and task-specific tests.
...
- After completing or blocking on the task, replace `docs/as2k/CODEX_RESULT.md` with a concise factual report containing commands/tests, observed values, files changed, and final `git status --short`.
