# Current Codex task — vertical cursor motion between two short rows

Date: 2026-09-13. Branch: as2k-mame0289-dev. Attempt 1.
Read AGENTS.md and mandatory private AS2K_KNOWLEDGE.md first.
Newline insertion, Backspace join, Left/Right newline traversal, F2/F1 and
separate-process recall are accepted. Preserve input, Send, XIRQ, banking and
LCD_RENDERING.md limits, including the earlier blank-LCD checkpoint and
unverified physical B5 glyph. Do not rediscover those closed test findings.

## One narrow task

Add bounded diagnostic coverage for Up/Down cursor motion between two short
rows separated by an explicit newline in F1. This is an unresolved coverage
gap, not a demonstrated emulator defect. Use fresh private NVRAM, asma2k input
definitions and established idle gating. Exclude wrapping, scrolling and
unequal-line-length clamping behavior from this task.

## Pass criteria and LOCAL validation

- Establish ab<CR>cd using the accepted split sequence, cursor before c.
  Require exact raw DDRAM rows ab<B5> / cd, padded to 40 cells.
- Up once then insert x: test expected xab<B5> / cd.
- Down once then insert y: test expected xab<B5> / cyd (column one).
  These are proposed firmware expectations, not verified physical behavior.
  Distinguish vertical movement from ignored input, horizontal movement or overwrite.
...
- Retain Left/Right traversal, newline/join, interior-edit, all-eight-file
  isolation/restart, production kKzZ=+, Send, diagnostic keyboard/LCD,
  six-character pixel, input fixture, HC11 harness, focused build, -validate
  and BIOS audit gates.
...
Serial builds passed after an earlier compiler ICE; its cause remains unproven.
