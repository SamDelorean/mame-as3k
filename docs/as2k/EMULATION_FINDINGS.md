...
The captured stream was `1C F0 1C 32 F0 32 21 F0 21 29 F0 29 16 F0 16 1E F0 1E 26 F0 26`. Decoding Set-2 make/break semantics yields exactly `abc 123`; `send.txt` contained bytes `61 62 63 20 31 32 33`. `tools/as2k_decode_wired_send.py` is the reproducible decoder used for this gate. This validates `$AA54/$0046` as a sufficient logical text-sink boundary while preserving the ROM's original translation and sequencing.
...
The validated `$8606 -> $AA54/$0046 -> $80F5` path is now integrated as a Send-scoped text sink. `$8606` opens/truncates `send.txt`, `$AA54` consumes only bytes emitted by the stock serializer while Send is active, and `$80F5` flushes/closes the file. The first decoder implements the validated Set-2 make/break subset needed for ordinary letters, digits, space, Return and Tab; unsupported codes are ignored pending evidence-backed expansion.

End-to-end regression used fresh NVRAM, explicitly set `PC Connected=OFF`, typed `abc 123` through the normal keyboard, restarted with `PC Connected=ON`, waited for `$80E5`, and pressed Send through `COL.7`. The ROM emitted 21 serializer bytes and the integrated sink produced `send.txt` containing exactly `abc 123` (`61 62 63 20 31 32 33`). State: `INTEGRATED_PC_TEXT_SINK_V1`.

## Integrated text sink V2: Shift, punctuation, Return and Tab (2026-09-17)

The integrated AS2000 text sink was extended from the initial lowercase/digit subset using bytes observed directly from the stock ROM's PC Send stream. The stock firmware uses Set-2 `0x12` as the Shift make code and `F0 12` as its release around uppercase letters and shifted punctuation. The validated punctuation make codes are: `54/5B` brackets, `4C` semicolon, `52` quote, `41` comma, `49` period, `4A` slash, `4E` minus, `55` equals, `5D` backslash and `0E` grave; their shifted forms produce the corresponding US-keyboard symbols.
